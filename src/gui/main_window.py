...
# Absolute Imports (korrigiert!)
from src.gui.components.collapsible_frame import CollapsibleFrame
from src.gui.components.resizable_pane import ResizablePane
from src.automation.acrobat_controller import AcrobatController
from src.automation.field_operations import FieldOperations
from src.utils.logger import Logger
from src.utils.file_handler import SettingsManager, FileHandler
...
class AcrobatFormAutomatorPro(ctk.CTk):
    def __init__(self):
        ...
        self.log_message(
            "Bereit für den Roboti GLXY Roboti Formular - Weltmeister?", "INFO"
        )
    ...
    def create_file_section(self):
        """Erstellt die Datei-Auswahl Sektion"""
        ...
        pdf_label = ctk.CTkLabel(
            content, text="Adobe Acrobat DC PDF:", font=ctk.CTkFont(weight="bold")
        )
        ...
        pdf_btn = ctk.CTkButton(
            pdf_frame, text="📂 PDF wählen", width=100, command=self.select_pdf
        )
        pdf_btn.pack(side="right", padx=10, pady=5)
        
        # Daten-Auswahl
        data_label = ctk.CTkLabel(
            content, text="Feld-Definitionen:", font=ctk.CTkFont(weight="bold")
        )
        ...
        data_btn = ctk.CTkButton(
            data_frame, text="📂 Daten laden", width=100, command=self.select_data
        )
    ...
    def create_main_functions(self):
        """Erstellt die Hauptfunktionen-Sektion"""
        # Hauptfunktionen Frame
        main_frame = CollapsibleFrame(
            self.left_scrollable, title="🚀 Hauptfunktionen"
        )
    ...
    def create_field_creation(self):
        """Erstellt die Feld-Erstellung Sektion"""
        # Feld-Erstellung Frame
        field_frame = CollapsibleFrame(
            self.left_scrollable, title="📝 Feld-Erstellung"
        )
        ...
        type_label = ctk.CTkLabel(
            content, text="Feldtyp:", font=ctk.CTkFont(weight="bold")
        )
        ...
        # Eingabefelder
        ctk.CTkLabel(props_frame, text="Feldname:").grid(
            row=0, column=0, sticky="w", padx=5, pady=5
        )
        self.field_name_entry = ctk.CTkEntry(
            props_frame, placeholder_text="Feldname eingeben"
        )
        self.field_name_entry.grid(row=0, column=1, sticky="ew", padx=5, pady=5)
        
        ctk.CTkLabel(props_frame, text="Anzeigename:").grid(
            row=1, column=0, sticky="w", padx=5, pady=5
        )
        self.display_name_entry = ctk.CTkEntry(
            props_frame, placeholder_text="Anzeigename"
        )
        ...
        capture_btn = ctk.CTkButton(
            btn_frame, text="📍 Position erfassen", command=self.capture_position
        )
        capture_btn.pack(side="left", padx=5)
        
        create_btn = ctk.CTkButton(
            btn_frame, text="➕ Feld erstellen", command=self.create_single_field
        )
        create_btn.pack(side="right", padx=5)


    def create_settings_section(self):
        """Erstellt die Einstellungen-Sektion"""
        # Einstellungen Frame
        settings_frame = CollapsibleFrame(
            self.left_scrollable, title="⚙️ Einstellungen"
        )
        ...
        speed_label = ctk.CTkLabel(
            content,
            text="RobotiRoboti-Geschwindigkeit:",
        ...
        speed_slider = ctk.CTkSlider(
            content, from_=0.2, to=3.0, variable=self.speed_var
        )
        ...
        safety_check = ctk.CTkCheckBox(
            safety_frame, text="Sicherheitsmodus", variable=self.safety_var
        )
        safety_check.pack(anchor="w", padx=10, pady=5)
        
        self.backup_var = ctk.BooleanVar(value=True)
        backup_check = ctk.CTkCheckBox(
            safety_frame, text="Auto-Backup", variable=self.backup_var
        )
        ...
        self.pause_btn = ctk.CTkButton(
            control_frame, text="⏸️ Pausieren", command=self.toggle_pause
        )
    ...
    def create_calibration_section(self):
        """Erstellt die Kalibrierungs-Sektion"""
        # Kalibrierung Frame
        calib_frame = CollapsibleFrame(
            self.left_scrollable, title="📐 Tool-Kalibrierung"
        )
        ...
        for text, tool_name in tools:
            ...
            status_label = ctk.CTkLabel(
                tool_frame, text="Nicht kalibriert", text_color="#ff6b6b"
            )
    ...
    def create_status_section(self):
        """Erstellt die Status-Sektion"""
        # Status Frame (minimierbar und resizable)
        status_frame = CollapsibleFrame(
            self.left_scrollable, title="📊 Status & Protokoll"
        )
        ...
        self.task_label = ctk.CTkLabel(
            content, text="Aktuelle Aufgabe: Bereit", font=ctk.CTkFont(weight="bold")
        )
        ...
        self.status_label = ctk.CTkLabel(
            content,
            text="Status: Bereit für Automatisierung",
            fg_color="#4CAF50",
            corner_radius=5,
        )
        ...
        log_label = ctk.CTkLabel(
            log_pane, text="📋 Aktivitätsprotokoll", font=ctk.CTkFont(weight="bold")
        )
        log_label.pack(anchor="w", padx=10, pady=5)
        
        # Log-Text mit Scrollbar
        self.log_text = ctk.CTkTextbox(
            log_pane, height=150, font=ctk.CTkFont(family="Consolas")
        )
        ...
        clear_btn = ctk.CTkButton(
            log_btn_frame, text="🗑️ Löschen", width=80, command=self.clear_log
        )
        clear_btn.pack(side="left", padx=5)
        
        save_btn = ctk.CTkButton(
            log_btn_frame, text="💾 Speichern", width=80, command=self.save_log
        )
        save_btn.pack(side="right", padx=5)


    def create_right_panel(self):
        """Erstellt das rechte PDF-Viewer Panel"""
        ...
        open_pdf_btn = ctk.CTkButton(
            pdf_controls, text="📂 PDF öffnen", command=self.open_pdf_in_viewer
        )
        open_pdf_btn.pack(side="left", padx=5)
        
        reload_btn = ctk.CTkButton(
            pdf_controls, text="🔄 Neu laden", command=self.reload_pdf
        )
        reload_btn.pack(side="left", padx=5)
        
        # PDF-Viewer Bereich
        self.pdf_viewer_frame = ctk.CTkFrame(self.right_panel, fg_color="white")
        self.pdf_viewer_frame.grid(
    ...
    def create_pdf_placeholder(self):
        ...
        icon_label = ctk.CTkLabel(
            placeholder_frame, text="📄", font=ctk.CTkFont(size=80)
        )
    ...
    def open_pdf_in_viewer(self):
        ...
        if not self.pdf_path:
            messagebox.showwarning(
                "Keine PDF", "Bitte wählen Sie zuerst eine PDF-Datei aus."
            )
            return
        ...
        try:
            ...
            if PDF_VIEWER_AVAILABLE:
                ...
                self.log_message(
                    f"PDF geöffnet: {os.path.basename(self.pdf_path)}", "SUCCESS"
                )
            else:
                ...
                self.log_message(
                    "PDF-Viewer nicht verfügbar - CTkPDFViewer installieren. Ansonsten Pech gehabt.",
                    "WARNING",
                )
    ...
    def preview_data(self):
        ...
        header_label = ctk.CTkLabel(
            preview_window,
            text="📊 Feld-Definitionen Vorschau",
        ...
        if not field_types.empty:
            stats_text += " | " + " | ".join(
                [f"{t}: {c}" for t, c in field_types.items()]
            )
        
        stats_label = ctk.CTkLabel(
            preview_window, text=stats_text, font=ctk.CTkFont(size=12, weight="bold")
        )
        stats_label.pack(pady=10)
        
        # Schließen Button
        close_btn = ctk.CTkButton(
            preview_window, text="✅ Schließen", command=preview_window.destroy
        )
    ...
    def _create_fields_worker(self, df):
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                self.log_message(
                    f"Erstelle Feld {index+1}/{total}: {field_name}", "INFO"
                )
                ...
                try:
                    ...
                    if success:
                        successful += 1
                        self.log_message(
                            f"✅ '{field_name}' erfolgreich erstellt", "SUCCESS"
                        )
                    else:
                        failed += 1
            ...
            self.log_message(
                f"❌ Fehler: {failed} Felder", "ERROR" if failed > 0 else "INFO"
            )
    ...
    def rename_all_fields(self):
        ...
        if not self.data_path:
            messagebox.showwarning(
                "Keine Daten", "Bitte laden Sie zuerst Feld-Definitionen."
            )
            return
        ...
        result = messagebox.askyesno(
            "Felder umbenennen", f"🔄 Sollen {len(df)} Felder umbenannt werden?"
        )
        if not result:
            return
    ...
    def _rename_fields_worker(self, df):
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                self.log_message(
                    f"Suche Feld {index+1}/{total}: '{original_name}'", "INFO"
                )
                self.update_progress(index / total)
                
                try:
                    if self.field_operations.find_and_rename_field(
                        original_name, new_name
                    ):
                        successful += 1
                        self.log_message(
                            f"✅ '{original_name}' → '{new_name}'", "SUCCESS"
                        )
                    else:
                        failed += 1
                        self.log_message(
                            f"❌ Feld '{original_name}' nicht gefunden", "WARNING"
                        )
                
                except Exception as e:
                    failed += 1
                    self.log_message(
                        f"❌ Fehler bei '{original_name}': {str(e)}", "ERROR"
                    )
            ...
            try:
                ...
                if position:
                    ...
                    self.after(
                        0,
                        lambda: messagebox.showinfo(
                            "Position erfasst", f"Position erfasst: {position}"
                        ),
                    )
    ...
    def create_single_field(self):
        ...
        if not hasattr(self, "captured_position") or not self.captured_position:
            messagebox.showwarning(
                "Keine Position", "Bitte erfassen Sie zuerst eine Position."
            )
            return
        ...
        if not field_name:
            messagebox.showwarning(
                "Fehlende Eingabe", "Bitte geben Sie einen Feldnamen ein."
            )
            return
        ...
        try:
            ...
            if success:
                ...
                messagebox.showinfo(
                    "Erfolg", f"Feld '{field_name}' erfolgreich erstellt!"
                )
            else:
                self.log_message(
                    f"❌ Fehler beim Erstellen von '{field_name}'", "ERROR"
                )
    ...
    def calibrate_tool(self, tool_name):
        ...
        def wait_for_calibration():
            try:
                ...
                self.log_message(
                    f"✅ {tool_display} kalibriert: ({x}, {y})", "SUCCESS"
                )
                
                # Update UI
                self.after(
                    0,
                    lambda: self.calibration_labels[tool_name].configure(
    ...
    def load_settings(self):
        ...
        try:
            ...
            self.log_message("Hinweis: Keine gespeicherten Einstellungen gefunden", "DEBUG")
